; _OcrUtils.ahk
; Utilidades genericas de OCR (motor nativo de Windows, Windows.Media.Ocr) para uso
; propio en scripts de este proyecto. Adaptado de las funciones genéricas de OCR.ahk
; (WinRT OCR wrapper, sin nada especifico del juego) para no tener que incluir ese
; archivo completo -- el archivo completo trae funciones (FindPackStats, CountShinedust)
; que dependen de un motor de busqueda de imagenes que no usamos aca.
;
; No se modifica ningun archivo de Kevin/Lean -- esto es una copia propia, aislada,
; de las partes genericas (nada de coordenadas ni logica de juego).
...
GetTextFromBitmap(pBitmap, charAllowList := "", lang := "FirstFromAvailableLanguages") {
    ocrText := ""
    hBitmap := Gdip_CreateHBITMAPFromBitmap(pBitmap)
    pIRandomAccessStream := HBitmapToRandomAccessStream(hBitmap)
    ocrText := ocr(pIRandomAccessStream, lang)
